...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `مقالات في تصنيف ${category} | نقل عفش الرياض بريمو موفرز`,
    description: `تصفح مقالاتنا ونصائحنا في تصنيف ${category} وخدمات نقل الأثاث في الرياض`,
  };
}
...
export async function generateStaticParams() {
    ...
    posts.forEach(post => {
      if (post.category) {
        categories.add(post.category.toLowerCase().trim());
      }
    });
    
    console.log("التصنيفات التي تم إنشاؤها:", Array.from(categories));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <header className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            تصفح مقالاتنا ونصائحنا في تصنيف {category} وخدمات نقل الأثاث في الرياض
          </p>
        ...
        {categoryPosts.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8">لم نعثر على أي مقالات في هذا التصنيف. يرجى تصفح التصنيفات الأخرى.</p>
            <Link 
              href="/blog"
